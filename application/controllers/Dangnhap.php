...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->data['title']='Phong Cách Đỏ - Đăng nhập tài khoản';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['smtp_user']    = 'yuhgamesing102@gmail.com';
            $config['smtp_pass']    = 'xeyxculwykgzftnd';
            ...
            $this->email->from($result[0]['mail_smtp'], 'Phong Cách Đỏ');
            $this->email->to($email);
            $this->email->subject('Hệ thống Phong Cách Đỏ - Quà thành viên mới');
            $this->email->message('Bạn đã trở thành thành viên mới của cửa hàng Phong Cách Đỏ, Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000 đ : '.$tempcoupon.'<br>Mã này có giá trị tới ngày '.$tempdatelimit.'<br>Hãy sử dụng tài khoản để mua hàng để tích lũy nhận thêm nhiều ưu đãi !!!! <br>Cảm ơn quý khách đã ghé thăm cửa hàng!');
            $this->email->send();
            $this->data['success']='Đăng ký thành công! Bạn đã nhận được 1 mã giảm giá cho thành viên mới, vui lòng kiểm tra email !!';

        }  
        $this->data['title']='Phong Cách Đỏ - Đăng ký tài khoản';   
    ...
    public function forget_password(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['validation'] = TRUE;   
            $this->email->initialize($config);
            $this->email->from('yuhgamesing102@gmail.com', 'Phong Cách Đỏ');
            $this->email->to($list['email']);
            $this->email->subject('Hệ thống Phong Cách Đỏ - Lấy lại mật khẩu');
        ...
        $this->data['title']='Phong Cách Đỏ - Quên mật khẩu';   
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
            ...
            $this->data['title']='Phong Cách Đỏ - Cập nhật mật khẩu mới';
