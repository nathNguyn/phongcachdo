...
class Khachhang extends CI_Controller {
    ...
    public function index(){
        $this->data['title']='Phong Cách Đỏ - Quản Lý Sản Phẩm Phụ Kiện Điện Tử, Laptop & Máy Tính!';
	...
	public function postProduct()
	{
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if(empty($_POST['catid']) || empty($_POST['name']) || empty($_POST['detail']) || empty($_POST['sortDesc']) || empty(strval($_POST['number'])) || empty($_POST['price_root']) || empty($_POST['price_buy'])){
                $this->data['title']='Phong Cách Đỏ - Đăng Bán Sản Phẩm Phụ Kiện Điện Tử, Laptop & Máy Tính!';
                $this->data['view']='post';
    ...
    public function updateProduct($id)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if(empty($_POST['catid']) || empty($_POST['name']) || empty($_POST['detail']) || empty($_POST['sortDesc']) || empty(strval($_POST['number'])) || empty($_POST['price_root']) || empty($_POST['price_buy'])){
                $this->data['title']='Phong Cách Đỏ - Cập Nhật Sản Phẩm Phụ Kiện Điện Tử, Laptop & Máy Tính!';
    ...
    public function importNumber($id)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if(empty($number)){
                ...
                $this->data['title']='Phong Cách Đỏ - Nhập Số Lượng Sản Phẩm Phụ Kiện Điện Tử, Laptop & Máy Tính!';
                return $this->load->view('frontend/layout',$this->data);
            }
